...
class Countdown(unittest.TestCase):
    def test_workspace_guidance_has_one_clear_role_per_screen(self):
        import app
        self.assertEqual(len(app.WORKSPACE_ITEMS), 6)
        self.assertEqual([item[2] for item in app.WORKSPACE_ITEMS], [
            'COUNTER · EVENT DAY', 'BOOKINGS · RECORD DESK', 'SCHEDULE · PRIEST LIST',
            'DEVOTEES · FAMILY REGISTER', 'REPORTS · REVIEW & EXPORT', 'MASTERS · ADMIN SETUP'])
